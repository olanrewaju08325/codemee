        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
          {deadlines.map((deadline, index) => (
            <div key={index} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)', padding: 'var(--space-3)', backgroundColor: 'var(--bg-primary)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-default)' }}>
              <div style={{ fontSize: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {deadline.icon || '📝'}
              </div>
              ...
              <div style={{ fontSize: 'var(--text-xs)', fontWeight: 'var(--weight-bold)', color: deadline.color || '#F59E0B', backgroundColor: `${deadline.color || '#F59E0B'}15`, padding: '4px 8px', borderRadius: 'var(--radius-sm)', whiteSpace: 'nowrap' }}>
                {deadline.due}
              </div>
            </div>
          ))}
